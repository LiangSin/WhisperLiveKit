...
# ---------------------------------------------------------------------------
# Rolling-hash looping-repetition detector
# ---------------------------------------------------------------------------

class LoopingDetector:
    """Online detector for substrings repeated ``MIN_REPEATS``+ consecutive times.

    Algorithm
    ---------
    Core observation: if a repetition of ``MIN_REPEATS`` copies ends at
    position ``n-1``, its last copy must end exactly at ``n``.  So after
    appending each character we only need to enumerate candidate period
    lengths ``L`` (1 … MAX_PERIOD) and check whether the last ``MIN_REPEATS``
    windows of length ``L`` all share the same rolling hash.
    ...
    it is truncated to the last ``MAX_PERIOD * MIN_REPEATS`` chars and the
    hash tables are rebuilt in O(MAX_PERIOD) time.
    """

    BASE: int = 131
    MOD: int = (1 << 61) - 1   # 8th Mersenne prime — collision prob ≈ 1/2^61
    MAX_PERIOD: int = 50        # longest pattern period to detect
    MIN_REPEATS: int = 4        # minimum consecutive repetitions to trigger detection
    ...
    def _trim(self) -> None:
        """Retain only the last ``MAX_PERIOD * MIN_REPEATS`` chars and rebuild prefix tables."""
        keep = self.MAX_PERIOD * self.MIN_REPEATS
    ...
    def feed(self, text: str) -> tuple[bool, Optional[str]]:
        """Append *text* one character at a time and report the first looping
        repetition (``MIN_REPEATS`` consecutive copies of the same pattern)
        found ending at any position in this batch.
        ...
        for ch in text:
            ...
            n = len(self._text)
            for L in range(1, min(n // self.MIN_REPEATS, self.MAX_PERIOD) + 1):
                if all(
                    self._hash(n - (k + 1) * L, n - k * L)
                    == self._hash(n - L, n)
...
def contains_hallucination(
    ...
    phrases: list,
    *,
    detector: Optional[LoopingDetector] = None,
    new_text: str = "",
    ...
    1. Incremental looping-repetition (only when `detector` and `new_text`
       are provided): feeds new_text into the rolling-hash detector and
       returns immediately if a substring repeated ≥ ``LoopingDetector.MIN_REPEATS``
       consecutive times is found.
    ...
    """
    # --- check 1: looping repetition (incremental) ---
    if detector is not None and new_text:
        is_repeat, pattern = detector.feed(new_text)
        if is_repeat:
            return True, f"{pattern!r} ({LoopingDetector.MIN_REPEATS}x repeat)"
