...
class TextDecoder(nn.Module):
    ...
    def forward(
        ...
        # Without this, offset + len(x) can exceed the positional embedding length on
        # long-lived connections, crashing the positional embedding lookup.
        max_total_length = self.positional_embedding.shape[0]
        if offset + x.shape[-1] > max_total_length:
            if kv_cache:
                # Drop the whole cache (self- and cross-attention); the blocks
                # below re-project and re-cache K/V from scratch this pass.
                kv_cache.clear()
                offset = 0
