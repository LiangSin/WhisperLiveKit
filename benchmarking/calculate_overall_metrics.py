...
def calculate_metrics(file_paths):
    ...
    for file_path in file_paths:
        ...
        if not isinstance(data, list):
            print(f"Error: JSON content in {file_path} must be a list of result objects.")
            sys.exit(1)
        ...
        for i, item in enumerate(data):
            ...
            if 'error_ratio' not in item:
                print(
                    f"Warning: Item {i} in {file_path} "
                    f"(id: {item.get('id', 'unknown')}) missing "
                    "'error_ratio' field. Skipping."
                )
                continue
    ...
    print("-" * 40)
    print(f"Overall Statistics ({count} samples across {len(file_paths)} file(s))")
    print("-" * 40)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "file_paths",
        nargs="+",
        help="Paths to one or more benchmark results JSON files.",
    )
    
    args = parser.parse_args()
    calculate_metrics(args.file_paths)
